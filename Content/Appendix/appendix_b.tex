...
\begin{align}
	\min_{x, y} \quad z_\mt{3F}(x, y) & =  \sum_{k = 1}^K \sum_{(i, j) \in E} d_{ij} x_{ijk} \label{eq:three-index-flow-obj-func}                                                                                                                          \\
	                                  ...
	                                  & \sum_{i, j \in \delta(S)} x_{ijk} \ge 2 y_{hk}                                            & \quad \EqStackTwo{\forall S \subseteq V_0, |S| \ge 2}{k = 1, \dots, K} \label{eq:three-index-flow-ccc}                 \\
...
Constraints \eqref{eq:three-index-flow-ccc} are the TSP subtour elimination constraints (SECs) and are used to avoid the formation of spurious unconnected subtours.
...
Constraint \eqref{eq:three-index-flow-ccc} can be rewritten in the traditional (non-generalized) Subtour Elimination Constraint (SEC) form:

\begin{equation}\label{eq:three-index-flow-gsec}
