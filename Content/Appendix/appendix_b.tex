...
is an IP model employing $O(\frac{N (N+1)}{2} K + N K)$ binary variables.

We define a set of binary variables $x_{ek} \in \Set*{0, 1}, e = \Set*{i, j} \in E, k = 1, \dots, K$, to encode whether vehicle $k$ traverses edge $e$,
...
\begin{align}
	\min_{x, y} \quad z_\mt{3F}(x, y) & = \sum_{k = 1}^K \sum_{e \in E} d_{e} x_{ek} \label{eq:three-index-flow-obj-func}                                                                                                                               \\
	                                  ...
	                                  & \sum_{e \in \delta(i)} x_{ek} = 2 y_{ik}                                          & \quad \forall i \in V,\ k = 1, \dots, K \label{eq:three-index-flow-force-visited-customer-if-flow}                          \\
	                                  & \sum_{i \in V} q_i y_{ik} \le Q                                                   & \quad k = 1, \dots, K \label{eq:three-index-flow-force-resource-upper-bound}                                                \\
	                                  & \sum_{e \in \delta(S)} x_{ek} \ge 2 y_{hk}                                        & \quad \EqStackTwo{\forall h \in S,\ \forall S \subseteq V_0,\ |S| \ge 2}{k = 1, \dots, K} \label{eq:three-index-flow-secv1} \\
	                                  & x_{ek}                   \in \Set*{0, 1}                                          & \quad \forall e \in E,\ k = 1, \dots, K             \label{eq:three-index-flow-x-mip-var-bounds}                            \\
...
\begin{equation}\label{eq:three-index-flow-secv2}
	\sum_{e \in E(S)} x_{ek} \le |S| - 1 \quad \forall S \subseteq V_0, |S| \ge 2,\ k = 1, \dots, K
\end{equation}
