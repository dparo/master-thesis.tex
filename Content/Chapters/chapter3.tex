...
\begin{itemize}
	...
	\item Controlling the RCSP dynamic labeling pricer:
	      \begin{itemize}
		      ...
		      \item \texttt{CutTailingOffThreshold = 0.0000001}, \texttt{CutTailingOffCounterThreshold = 999999}, named $\delta^\mt{gap}, \delta^\mt{num}$ in the original paper, controls the tailing off condition at which BapCod deems cuts ineffictive and threfore switches to lower priority cuts which are usually more computationally expensive.
		            Despite their name, these two parameters also affect the pricing algorithm, as pointed in the original form, increasing the ng-sets can be seen as a form of "cut generation" since it improves the dual bound.
		            Since we are not interested in cut generation, and we want BapCod to always prefer augmenting the ng-sets, these two parameters to disable the tailing off condition.
	      \end{itemize}
\end{itemize}
