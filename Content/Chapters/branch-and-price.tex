...
as the non-elementary version of the ESPPRC.
...
this approach has flaws when the constructed routes visit numerous nodes,
...
First, the duality gap obtained at the linear relaxation is significantly better
than when treating $q$-routes.
...
Some major contributions in this domain can be attributed to:
(i) \textit{bidirectional search} proposed in \textcite{righini2006},
(ii) \textit{decremental state space relaxation} (DSSR) proposed independently in
...
The dual variables $\pi \in R^N$ tend to oscillate from one iteration to the next
...
We conclude the section by mentioning two more surveys that
discuss the labeling algorithm for solving the
Shortest Path Problem with Resource Constraints (SPPRC):  \textcite{irnich2005, pugliese2010, pugliese2013}.
