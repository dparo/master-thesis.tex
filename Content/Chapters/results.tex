...
The second version of the algorithm is less aggressive and it is known as \texttt{BAC AFL} (Amortized Fractional Labeling).
...
The results in \cref{fig:perfprofs-batch1-part1} were obtained using a single core and 60 seconds of timelimit.
The BAC-pricer version that produces tighter bounds will be the winner.
\Cref{fig:perfprofs-batch1-part1} highlights that \texttt{BAC AFL} produces tigther dual bounds in the majority of cases.
As a result, in the second group of empirical results we will compare only the \texttt{BAC AFL} version against the labeling algorithm.

\medskip

\mytodo{Report timelimit, num threads of the MIP optimizer}
