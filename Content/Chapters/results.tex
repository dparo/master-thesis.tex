...
We tested the performance of the two pricers approach for different test sets under progressively higher vehicle capacities.
...
\Cref{fig:perpfrofs-batch2-E-part1,fig:perpfrofs-batch2-E-part2} depicts the performance
of the pricers on the \texttt{E} test-set for different values of the inflation scale factor
$s \in \Set*{1, 2, 4, 5, 8, 10, 20}$.
At $s = 5$, the BAC-pricer begins to be competitive with the labeling algortihm.
From $s \in \Set*{8, 10, 20}$, not only is the BAC-pricer faster than the labeling algorithm,
but feeds substantially better dual bounds to the BPC framework.
