...
We've parameterized \bapcod\ as to lay a fair test bed for comparing
the performance of our branch-and-cut framework against the VRPSolver extension's
labeling algorithm.
...
First, we time the running time of the labeling algorithm in order to assess its performance later.
Second, at each pricing invocation we record the dual variables $\pi \in \R^N$ of the RMP
to later be dumped into a dedicated file per pricing invocation.

Due to some implementation details of \bapcod the reduced cost of an
...
from the reduced cost of each edge.
\bapcod\ follows the following convention to encode the reduced cost of an edge:
...
The CPTP instance is encoded in a slight modification of the TSPLIB95 file format.
