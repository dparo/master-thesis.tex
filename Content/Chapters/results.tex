...
In the case of the \texttt{A} test set, the BAC-pricer is not competitive at $s \in \Set*{1, 2, 4}$ and sometimes fails to complete the pricing problem within the time limit.
At $s = 5$ is competitive in terms of running time and it improves as we increase $s$.
No appreaciable dual bound improvements to the BPC framework can be observed when using the BAC approach.
