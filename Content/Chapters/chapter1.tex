...
Altought non strictly-necessary, it is possible to improve the linear relaxation of the IP model by introducing a static constraint modeling the lower bound on the capacity:

\begin{equation}
	\sum_{i \in V} d_i y_i   \ge B                    \label{eq:resource-lower-bound-constraint}
\end{equation}

where $B \in \R, B \ge 0$ is a constant and represents the lower bound on the served demand.
The constant $B$ value can be computed as $B = d_u + d_v$, where $u, v \ge 1$ represent the least two demanding customers among all the customers.
...
We will concentrate mostly on the most important cuts, additional cuts and information can be found in \cite{Jepsen2014}.
