...
We first need to re-formulate the CPTP as an ESPPRC.
We define a new network formulation composed of $N + 1$ nodes, where the depot node is essentially splitted in two nodes: a source and a sink.
Let $0$ denote the depot source version of the depot node, and $N$ its duplicate playing the role of the sink vertex.
...
We can bind the new edges to the original CPTP edges with the following constraint:

\begin{equation}
	x_{0i} + x_{iN} \le 1 \quad \forall i \in V_0
\end{equation}

\mytodo{Continue writing about the method here}
