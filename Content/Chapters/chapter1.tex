...
Finally, we can rewrite the CPTP original formulation in the ESPPRC IP model:

\begin{align}
	\min_{x,y} \quad z(x, y) & =  \sum_{(i, j) \in E_d} \left( d_{ij} - \frac{p_i + p_j}{2} \right) e_{ij} \label{eq:espprc-obj-function}                                                                                \\
	                         & y_0 = 1                                                                                                    & \label{eq:espprc-depot-part-of-tour-constraint}                              \\
	                         & \sum_{j \in \delta(i)}       x_{ij}  = 2 y_i                                                               & \quad \forall i \in V         \label{eq:espprc-flow-conservation-constraint} \\
	                         & \sum_{(i, j) \in \delta(S)} x_{ij} \ge \sum_{\substack{j \in V_d                                                                                                                          \\ j \ne i}} e_{ji} + \sum_{\substack{j \in V_d \\ j \ne i}} e_{ij}   & \quad \substack{\forall i \in S                                              \\ \forall S \subseteq V_0,\ |S| \ge 2} \label{eq:esppr-cgsec-constraints} \\
	                         & \sum_{i \in V} q_i y_i   \le Q                                                                             & \label{eq:espprc-resource-upper-bound-constraint}                            \\
	                         & x_{ij}                   \in \lbrace 0, 1 \rbrace                                                          & \quad \forall (i, j) \in E               \label{eq:espprc-x-mip-var-bounds}  \\
	                         & y_{i}                    \in \lbrace 0, 1 \rbrace                                                          & \quad \forall i \in V,\ i \ge 1          \label{eq:essprc-y-mip-var-bounds}
\end{align}
