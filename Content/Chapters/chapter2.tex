...
\subsection{Integral separation}\label{sec:integral-sep}
...
By recalling the GSEC inequality as presented in Equation \eqref{eq:gsec-constraints}, and the connected components as discussed in Section \ref{sec:integral-sep}, and in Algorithm \ref{algo:cc-dfs} we can pick:

\begin{equation}
	S \subseteq V_0 = \left\{ i \mid cc(i) = k  \right\}   \quad \forall k \in C, k > 0
\end{equation}

and since we are dealing with integer CPTP solutions of the form $x^*_{ij},\ y^*_{i} \in \{0, 1\}$ it is easy to see that
