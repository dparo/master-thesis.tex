...
First attempts, in solving the CVRP were based on branch-and-cut algorithms
which had historical prevalence.
Some notable BAC contributions are provided in the works of
\textcite{araque_g_branch-and-cut_1994}, \textcite{augerat_computational_1995}, \textcite{blasum_application_2000}, \textcite{achuthan_improved_2003}, \textcite{ralphs_capacitated_2003}, \textcite{lysgaard2004new},
...
Then, \textcite{fukasawa_robust_2006} solved all CVRP instances up to approximately 100 customers
using a branch-cut-and-price approach.
Later, the works of \textcite{baldacci2008exact}, \textcite{baldacci2011new}
improved the overall running times by employing a column and cut generation scheme.
Later, \textcite{contardo_new_2014} was able to solve a CVRP instance made of 151 (M-n151-k12).
Later, \textcite{pecin2017improved} solved CVRP instances made of 200 customers (M-n200-k16, M-n200-k7).
...
is that they are usually developed and tuned using historical
benchmark instances.
The historical test instances are classified in different sets (or families), and
are usually referred in the literature through a single upper case letter:
(i) sets A, B, P proposed in \textcite{augerat_computational_1995},
(ii) set E proposed in \textcite{christofides_algorithm_1969},
(iii) set F proposed in \textcite{fisher_optimal_1994},
(iv) set M proposed in \textcite{christofides_vehicle_1979}.
Another popular tested is the one proposed in \textcite{golden_impact_1998},
containing large scale instances ranging from 200 to 480 customers,
...
In the work of \textcite{uchoa2017new}, for example, they propose
a new more modern set of benchmark instances to replace the historical
used ones.
Despite some efforts in proposing new instances for the CVRP, the scientific community
has not agreed on a common denominator for new more-modern benchmark set, and therefore,
the common and historical instances remain the main central test-bed for comparing
...
This thesis aims at providing a first proof-of-concept answer
to whether it is better to employ a Branch \& Cut framework to solve the pricing problem
