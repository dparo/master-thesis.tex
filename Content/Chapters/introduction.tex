...
Branching occurs when either the optimal solution of the PP, $p^\star$, achieves $c^\star_p \ge 0$ and $\exists p \in \mc{P} \mid \lambda_p \notin \Set*{0, 1}$, or, the column generation procedure tails off, meaning the gained dual bound improvements compared to the running time to generate a column become suboptimal.
