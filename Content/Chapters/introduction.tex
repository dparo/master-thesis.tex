...
\subsection{The two-index vehicle flow formulation}
\label{sec:intro-cvrp-two-index-flow-formulation}

The two-index vehicle flow formulation was first presented in \cite{laporte1983, laporte1985} for the symmetric case,
...
The two-index vehicle flow formulation is usually solved through a branch-and-cut framework.
...
(ii) constraint the vehicle capacity, by imposing that any customer set $S$ is crossed by a number of edges not smaller than $r(S)$, which
represent the minimum number of vehicles needed to serve set $S$.
...
by means of the degree constraints \eqref{eq:two-index-flow-two-edges-incident-per-customer, eq:two-index-flow-two-k-edges-incident-in-the-depot-node}:
...
\subsubsection{The three-index vehicle flow formulation}
It needs mentionining, that to model more "colorful" variations of the CVRP, the two-index vehicle flow model may not be  sufficient.
A very popular formulation to model complex variations of the CVRP is the three-index vehicle flow formulation,
preseted in \textcite{toth2002}.

We quickly recap the model here.
The three-index vehicle flow formulation makes use $O(N^2 K + N K)$ binary variables.
...
The three-index vehicle flow formulation can be useful to model the CVRP variant
where the fleet of trucks is heterogenous, i.e. the capacity
...
works substantially differently from the two-index vehicle flow formulation or many other commonly known IP formulations.
...
The SP formulation can be viewed as a Dantzig-Wolfe reformulation \parencite{dantzig1960} of the two-index vehicle flow formulation.
...
Transformign the SP to the SC formulation vastly shrinks the size of the dual solution space.
