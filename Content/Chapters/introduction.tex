...
We can therefore describe the two-index flow formulation as an Integer Programme (IP) model:
...
\begin{align}
    \min_{x} \quad z_\mt{2F}(x) & =  \sum_{(i, j) \in E} d_{ij} x_{ij} \label{eq:two-index-flow-obj-func}                                                                                                                          \\
                                ...
                                & \sum_{j \in V_0} x_{0i} = 2 K                                                        & \label{eq:two-index-flow-two-k-edges-incident-in-the-depot-node}                                                             \\
...
of constraints (see \cite{miller1960, christofides1979, desrochers1991}), but these
models have shown to provide weak formulations,
while the GSEC constraints are in the TSP case facet-defining.
