...
In this section we will introduce the families of cuts and cut generation for the SP formulation,
under the assumption that a branch-and-price scheme is employed
for solving the CVRP.

The very few first attempts in proposing valid inequalities
to improve the linear relaxation for the CVRP were obtained by
generalizing the constraints for the traditional TSP problem, see \textcite{naddef1993}.
\citeauthor{naddef1993} showed that its is possible to re-adapt to the CVRP,
by putting into a tight triangular form,
every valid inequality obtained for the TSP.

As is done in \textcite{toth2014}, families of valid robust inequalities for the CVRP can
...
\begin{itemize}
	...
	\item \textbf{Multistar Inequalities}, proposed in \textcite{araque1990}.
	      ...
	      for each value of $x(E(S))$ an upper bound on the $x(\delta(S) \cap \delta(T))$.
