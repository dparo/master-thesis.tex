...
In q-routes we exclude cycles of two vertices
and each additional visit to each customer
still counts to the total consumption of the resource.
...
This enlargement while still valid, it has unfortunately the downside
of obtaining significantly worse dual bounds for the linear relaxations.
...
Therefore, the pricing problem in column generation techniques
has been modeled by many authors as an SPPRC, i.e.
relaxing the elementarity condition.
...
As \citeauthor{feillet2004} points out,
...
Although some contributions have been made to improve its overall running time.
...
approach where elementarity is iteratively added for each violated node.
...
algorithm employ it as an heuristic pricer.
