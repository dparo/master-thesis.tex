...
In the VRPTW \parencite{schrage1981} each customer is associated with a time window slot in which it can be served,
...
\begin{enumerate}
	...
	\item The three-index arc flow formulation \parencite{toth2002}.
...
and the development of efficient branch-and-price (\textbf{BPC}) frameworks, it has become the de facto
...
We assume an Euclidian CVRP problem, i.e. the distance function is symmetric $c_e = c_{ij} = c_{ji}$ and satisfies the triangle inequality $c_{ij} \le c_{ih} + c_{hj}$.
...
The two-index arc flow formulation is tradiionally tackled with branch-and-cut frameworks.
...
where $z_\mt{2F}(x)$, as defined in \eqref{eq:two-index-flow-obj-func}, is the objective function that aims at minimizing the overall routing cost (travel time).
...
Constraints \eqref{eq:two-index-flow-x-mip-var-bounds-depot}\eqref{eq:two-index-flow-x-mip-var-bounds} forces each edge to be traversed at most once,
except for all edges incident at the depot.
In fact, the edge case modeled in constraint \eqref{eq:two-index-flow-x-mip-var-bounds-depot} is necessary to allow single-customer routes.
...
By setting $r(S) = \ceil*{q(S) / Q}$ in \cref{eq:two-index-flow-ccc-bpp-lb}
is alone sufficient to solve to optimality the two-index arc flow formulation.
Although, a better lower bound for the Bin Packing Problem can improve the linear
relaxation, thus reducing the number of branching events.
...
Instead, an appropriate cutting-plane algorithm and separation procedures may be employed to generate dynamically only the required
...
\begin{equation}
	{c^\star_p} = \min_{p \in P} \Set*{ \bar{c_p} = \sum_{(i, j) \in E} \Expr*{c_{ij} - \frac{\pi_i + \pi_j}{2}} a_{ijp}  }
\end{equation}
...
The oracle needs to solve the pricing subproblem which, due to the definition of the set $P$, coincides with solving an Elementary Shortest Path Problem with Resource Constraints (ESPPRC) over a directed graph with weights $\bar{c_{ij}} = c_{ij} - \frac{1}{2} \pi_i - \frac{1}{2} \pi_j$, where the source and sink vertex collapse at the depot node.
