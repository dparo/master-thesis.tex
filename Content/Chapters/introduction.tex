...
to the VRP can be found in the works of \textcite{costa2019}.
...
problem has been modeled by many authors as an SPPRC \parencite{feillet2004, contardo2011}.
...
$k$-cycle elimination with $k \ge 2$ within the labeling algorithm \parencite{irnich2006}.
Unfortunately, \textcite{fukasawa2006} analyze the $q$-routes with $4$-cycle elimination
and they show that already at this point the pricing becomes too slow
and the obtained dual bounds are still not satisfactory close to the elementary bounds.
...
and were confied into relatively small clusters of the graph.
They define the concept of ng-set $N_i$ for a vertex $i$
which encodes the $|N_i|$ closest neighborhood of the vertex $i$.
The quantity $|N_i|$ is pre-determined and can be raised on demand.
...
may be seen as a softer, versatile, and more dynamic form of $q$-routes
with $k$-cycle elimination.
They achieve a bound very close to the elementary bound while
no being detrimental for the pricing problem.
...
to handle ng-sets.
In their approach the size of the ng-sets are increased dynamically
in order to seek for more effective dual bounds.
