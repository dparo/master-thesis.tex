...
(i) set E proposed in \textcite{christofides1969} (no description about the generation),
(ii) set M proposed in \textcite{christofides1979} (obtained by adding some instances from the E set),
(iii) set F proposed in \textcite{fisher1994} (obtained from an actual distribution problem of groceries in the city of Ontario),
(iv) sets A, B, P proposed in \textcite{augerat1995} (generated artificially: A is random, B is clustered, P generated from A,B,E by changing capacities).
Another popular tested is the one proposed in \textcite{golden1998},
...
used ones, since those have become "rather easy" for modern algorithms.
...
It \todo{Citation needed} has been shown that the performance of branch-and-price frameworks tends to drop as the length of the routes increases.
...
can perform better than the dynamic labeling algorithm employed \cite{pessoa2020},
especially when the optimal routes tend to be longer than usual.

One of the main issues in the work of \cite{pessoa2020}, is that
