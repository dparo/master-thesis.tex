...
\begin{comment}
\textcite{fiellet2004}
The standard approach to solve the SPPRC in practice is
based on dynamic programming and has a pseudopolyno-
mial complexity. Briefly, the principle is to associate with
each possible partial path a label indicating the consumption
of resources and to eliminate labels with the help of dom-
inance rules. In label correcting approaches, nodes are re-
peatedly treated and their labels extended. This approach is
an extension of the Ford-Bellman algorithm. It was pro-
posed by Desrochers [9] in the SPPRC context. Within this
approach, two strategies can be employed. In reaching al-
gorithms, labels on a node are extended to its successors. In
pulling algorithms, labels from its predecessors are pulled to
the node currently treated. Besides the label correcting
approach, the label setting a
\end{comment}
...
algorithm which works by label construction, propagation and correction
from node to node, and dominance rules are used prune
the prolieration of labels.
It was first presented for the non-elementarity SPPRC generating q-routes
in \textcite{desrochers1992} for the VRPTW under a Set Covering (SC) formulation.
...
The label setting algorithm, was later extended in \textcite{feillet2004}
to handle the full-elementarity condition of the ESPPRC problem for the VRPTW.

The dominance criteria was introduced in \cite{feillet2004} \inlinetodo{is it true??}.


\begin{comment}
\texcite{feillet2004}
[Feillet 2004 :: Abstract]
In this article, we propose a solution procedure for the
Elementary Shortest Path Problem with Resource Con-
straints (ESPPRC). A relaxed version of this problem in
which the path does not have to be elementary has been
the backbone of a number of solution procedures based
on column generation for several important problems,
such as vehicle routing and crew pairing. In many cases
relaxing the restriction of an elementary path resulted in
optimal solutions in a reasonable computation time.
However, for a number of other problems, the elemen-
tary path restriction has too much impact on the solution
to be relaxed or might even be necessary.
Many of the most efficient solution algorithms for vehi-
cle routing problems are based on a decomposition scheme
and rely on the solution of SPPRC instances as subprob-
lems.
The approach of Desrochers et al. [10] is computation-
ally very attractive because the SPPRC can be solved effi-
ciently with dynamic programming. However, this method
has a significant disadvantage, which is a weakening of the
lower bound,

\textcite{feillet2004}
[Feillet talks about the advantages that It has to consider elementary routes in ESPPRC versus standard SPPRC]
In this context, it has two main
advantages. First, it reduces the duality gap, compared to the
usual approaches that are based on the SPPRC bound.
Second, it enables the use of a column generation solution
methodology for some special problems or with some spe-
cial branching schemes that cannot rely on SPPRC solu-
tions.
\end{comment}
...
The q-routes are routes with total resource consumption
$q$ that respect the vehicle capacity but do not satisfy the elementarity condition,
and thus may contain cycles.
...
\textcite{contardo2014} proposed an effective enumeration scheme and employed the ng-routes effectively.
