...
The less tighter inequality of \cref{eq:two-index-flow-ccc-bpp-lb} is sufficient to optimally solve
...
A set of integer variables $x_{ek} \in \Set*{0, 1, 2},\ e = \Set*{i, j} \in E,\ k \in \mcK$ is now used to encode the number of times a truck $k$ traverses an $e \in E$.
...
Finally, constraint \eqref{eq:three-index-flow-y-mip-var-bounds} bounds and forces integrality for the $y{ik}$ binary variables.
...
However, the three-index arc flow formulation suffers a downside, compared to the two-index version when modeling standard homogeneous CVRPs.
This new formulation suffers from the multiplicity of the solution space.
Since the all the vehicles share the same capacity, new symmetric solutions can be obtained by simply permuting the identity $k \in \mcK$ of each truck.
