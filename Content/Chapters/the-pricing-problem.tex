...
The ESPPRC's underlying network may,
in the general case,
contain negative cost cycles,
which makes solving such problem NP-hard \parencite{dror1994}.
A dynamic programming algorithm to tackle the elementary version of the problem was proposed in \textcite{feillet2004}.
Traditionally however, in the context of pricing, the problem can be relaxed into an SPPRC,
and the problem can be solved in pseudo-polynomial time with a faster dynamic programming algorithm of \textcite{desrochers1992}.
Unfortunately, relaxing the elementarity condition leads to slower column generation and worsening of dual bounds,
see discussion in \cref{sec:intro-solving-the-pricing-problem}.
