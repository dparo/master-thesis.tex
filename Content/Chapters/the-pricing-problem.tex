...
Similarly, the source and sink vertex do not share direct connections with one another.

\medskip

More formally, the ESPPRC is defined over a network $G = \Tuple*{V\^prime, A^\prime}$,
where $V^\prime = V_0 \cup \Set*{0, N}$ denotes the set of vertices and $A^\prime$ the set of arcs.
...
As it was done for the CVRP, for each vertex $i \in V^\prime$ we associate
a resource consumption (or customer demand) $q_i \quad \forall i \in V$.
The resource consumption semantics associated to each customer
$i \in V_0$ remains unaltered  from the CVRP, while for the source and sink vertices
we respectively fix $q_{0} = q_{N} = 0$.
...
we have an associated weight $\bar{c}_{ij} \in \R \quad \forall \Tuple*{i, j} \in A^\prime$.
The weight $\bar{c}_{ij}$ represents the reduced cost of an arc
...
with $p_0 = 0, \ p_{u + 1} = N$
in which $\Set*{p_1, \dots, p_s} \subseteq V_0$ unique customers are visited.
...
$q_p = q(p) = \sum_{j=0}^{u} q_{p_j}$ satisfies $q(p) \le Q$.
The optimal path $p^\star$ minimizes the overall path reduced cost
...
Notice that the presented IP formulation makes the following equality hold:
\begin{equation}
