...
We also define $E^\prime(S) = \Set*{e = \Set*{i, j} = \Set*{j, i}\in E^\prime \mid i, j \in S}$
...
Let $p^\star = \Tuple*{0, i_1, \dots, i_s, N}$ denote the optimal path solution to problem \cref{eq:espprc-obj-function,eq:espprc-flow-conservation-constraint-customers,eq:espprc-flow-conservation-constraint-depot1,eq:espprc-flow-conservation-constraint-depot2,eq:espprc-sec-constraints,eq:espprc-x-mip-var-bounds}
