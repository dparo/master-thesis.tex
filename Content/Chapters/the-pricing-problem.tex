...
The ESPPCC network is characterized by $N_0 + 2$ nodes,
where $N_0$, denotes the total number of customers in the original CVRP problem.
Two additional vertices $s = 0, t = N$ are used to represent respectively the source and sink vertices.
...
As \textcite{beasley1989} proposes for the general ESPPRC,
...
In our case, however, since $A^\prime$ does not admit for in-arcs
and out-arcs for respectively the source and sink vertices,
\cref{eq:espprc-relaxed-elementarity}
can be replaced with the much simpler:
\begin{equation}
	\label{eq:esppcc-simpler-relaxed-elementarity}
	\ExprESPPOutgoingEdges{i} \le 1 \quad \forall i \in V \cup \Set*{s, t}.
\end{equation}
