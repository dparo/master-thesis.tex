...
the CPTP is easier to describe and its formulation can be provided
on an undirected symmetric network, reducing the total number of independent variables.
...
present many valid inequalities by readjusting the commonly known VRP's cutting-planes.
They study the competitiveness of their approach
along with the contributions achieved by each valid inequality kind.
Their results show that while a branch-and-cut framework based on CPTP may
not be competitive in most cases,
it seemed to perform better w.r.t. to dynamic programming
when the number of customers increases.
In conclusion,
\textcite{jepsen2014}
show that there may be value in further developing branch-and-price
frameworks in the context of pricing for  the CVRP.
...
Constraint \labelcref{eq:cptp-gsec-constraints} are the so-called Generalized Subtour Elimination Constraints (GSECs),
...
By following the original presentation of \textcite{jepsen2014},
