...
Constraint \eqref{eq:espprc-x-mip-var-bounds} forces each arc $\Tuple*{i, j} \in A^\prime$ to be traversed at most once.
...
\end{equation}
Therefore, despite a constant factor of $2$,
the quantity $\ExprESPPIngoingEdges{i} + \ExprESPPOutgoingEdges{i}$
can be used to determine whether customer $i \in V_0$ is covered
by the optimal path of the IP model.
