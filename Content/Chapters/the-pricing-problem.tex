...
As we've discussed in \cref{sec:intro-branch-and-price},
...
Therefore, its study is of fundamental importance in order to
...
The general definition of the ESPPRC allows for multiple resources definition
...
increase as the route traverses new edges or vertices.
...
which is bounded by a global quantity $Q$, the vehicle capacity.
...
Since in our work we are focusing our attention to the CVRP, instead
to a more general routing problem, we will focus most of our attention
to the ESPPCC problem.
It is important to acknowledge that, the general ESPPRC version of the problem
is required to model other variations of the vehicle routing problem,
such as the commonly studied VRPTW.

The Capacitated Profitable Tour Problem (CPTP) is another
combinatorial optimization problem which can be used to model
the pricing sub-problem for the column generation schemes
employed to solve the CVRP.
The CPTP asks for a minimum cost route instead of a minimum cost path,
...
Analogously, we will do the same for the CPTP in \cref{sec:the-capacitated-profitable-tour-problem},
where we will also discuss additional valid inequalities.

Before jumping directly to the discussion of these two problems,
we will first dedicate the next section (\labelcref{sec:the-pricing-problem-literature-review})
to review the small amount of literature available
on branch-and-cut approaches for pricing.
...
no exact separation procedures are currently known RCC inequalities,
for this reason,
