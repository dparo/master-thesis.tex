...
ESPRCC branch-and-cut algorithms have not been covered,
and this is probably to the inherent difficulty in modeling time windows constraints within IP formulations,
which are modeled as big-M constraints, which are inherently computationally unstable \parencite{jepsen2008branchandcut}.
...
While we have already glimpsed over this problem in \cref{sec:column-generation-and-pricing-problem,sec:intro-solving-the-pricing-problem},
we've never provided a formal description of the mathematical model.
In this section we consider the basic version of the ESPPRC,
i.e. non-robust inequalities at the RMP level are not considered
since these change the structure and formulation of the basic ESPPRC problem.
...
The ESPPRC asks for the determination of the shortest path on a weighted network
between two vertices $s, t$
subject to restrictions of the resource consumption.
In the ESPPRC problem the elementarity condition requires that such optimal path may visit at most once each vertex of the network.
The ESPPRC's underlying network may in the general case,
...
In the special case in which the ESPPRC in which a network with proven non-negative
...
Initial contributions for tackling the ESPPRC exploited this property.
The resource consumption constraint was relaxed through a Lagrangean relaxation
approach, and by employing a branch-and-bound scheme and an off-the-shelf
standard shortest path algorithm (e.g. Dijkstra \cite{sniedovich2006dijkstra})
to compute feasible dual solutions.
...
which limit the effectiveness of the approach in the context of pricing.
...
Our presentation of the ESPPCC readjusts the
\textbf{directed} symmetric network based ESPPRC formulation
provided in \textcite{beasley1989, toth2002, toth2014}.
Refer to \textcite{jepsen2008branchandcut} for an ESPPCC model defined over an undirected symmetric network.
However, it has to be noted that the undirected symmetric network is in the general case needed
to handle time-windows constraints.
