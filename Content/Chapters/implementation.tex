...
Integral inequalities' separation can be seen as a procedure
to dynamically generate mandatory constraints
that would otherwise be impossible to insert statically into a MIP model.


On some cases,
exact separation may be computationally very expensive
for some families of inequalities/constraints.
If computationally expensive inequalities
are not strictly required to guarantee the algorithm's correctness,
a heuristic separation procedure may be devised for their inclusion.
...
Vertices that share the same label belong to the same set $S \subseteq V_0$.

The GSECs are the only constraints which must be obligatorily
included through an exact separation procedure,
at least for the integral solutions of model
...
The $(u, v)$-min-cuts outputs a subset $S \subseteq V$ induced from $(u, v)$
minimizing the term $\sum_{e \in \delta(S)} x^\star_e$,
which constitutes a valid labeling of the network.
...
may be employed \parencite{jepsen2014}.
