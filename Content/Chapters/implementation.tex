...
\label{sec:impl-additional-bounds}

\mytodo{Merge this section into the full static model description}

\subsubsection{Resource Consumption Lower Bound}
\label{sec:impl-resource-consumption-lower-bound}
...
which follows closely the full IP formulation presented in \eqref{eq:cptp-obj-function} without the GSECs.
...
\label{sec:impl-lower-cutoff-value}
...
\begin{equation}
	z_\mt{CPTP}(x, y) \ge - \sum_{i \in V} p_i
\end{equation}

which describes a very weak lower bound, and in pretty much all cases, is easily improved in the preprocessing stage of a standard MIP solver.
