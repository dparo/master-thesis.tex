...
Despite CPLEX internally already implements
the separation of some families of general cutting-planes (see \cref{sec:introduction-to-cplex})
reporting additional inequalities,
...
On some cases, exact separation may be computationally very expensive for some family of inequalities.
In such a case,
if such inequalities are not strictly required to guarantee the algorithm's correctness,
a heuristic separation procedure may be devised.

\medskip

In our implementation branch-and-cut algorithm,
we are concerned in separating the
GSECs \labelcref{eq:cptp-gsec-constraints},
RCC \labelcref{eq:cptp-rcc-inequality},
GLM \labelcref{eq:cptp-glm-inequality}
families of inequalities.
All these families of inequalities share something in common:
the determination of a subset $S \subseteq V$.
A subset $S \subseteq V$ can be determined via an appropriate \textit{labeling algorithm},
which assigns a label (integer) to each vertex of the network.
Vertices that share the same label belong to the same set $S \subseteq V$.

The exact GSECs separation is mandatory to guarantee the correctness of the approach.
Their separation must be guaranteed at least for the integral solutions of
model
\labelcref{eq:cptp-static-model-0,eq:cptp-static-model-1,eq:cptp-static-model-2,eq:cptp-static-model-3,eq:cptp-static-model-4,eq:cptp-static-model-5}.
GSECs's exact integral separation is easily obtained by tracing the connected components
induced from the integral solution of the model.
Tracing the connected components can be used to determine subsets
$S \subset V | \sum_{e \in \delta(S)} x_e = 0$.

For fractional solutions, however the matter is a little-bit more complicated.
Let $x^\star \in \R^{|E|}$ denote the value of a fractional solution,
then,
...
The $(u, v)$-min-cuts seeks for the subset $S \subseteq V$ induced from $(u, v)$
which minimizes the term $\sum_{e \in \delta(S)} x^\star_e$.

An exact separation procedure of RCC inequalities is currently not known
\parencite{jepsen2014}, see discussion in \cref{sec:cptp-rcc}.
For the GLM fractional separation, \textcite{jepsen2014} proposes
to use solve a $(u, v=0)$-min-cuts on a flow network where capacities are $u$ dependent,
see discussion in \cref{sec:cptp-glm}.

\medskip

In our implementation, for separating the RCC and GLM inequalities
we opted for a different approach
compared to the one proposed in \textcite{jepsen2014}.
Our implementation employs the same labeling algorithms already employed for the
GSEC separation, as a sub-optimal heuristic separation of the RCC and GLM inequalities.
