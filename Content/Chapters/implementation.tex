...
MIP programs are solvable in various ways, but the most typical one is to use the \textbf{simplex algorithm} (for solving the continuous relaxation) and the use of the \textbf{branch-and-cut} algorithm (to handle integrality constraints).
The branch-and-cut algorithm, first introduced in \textcite{padberg1991branch}, is an hybridization between a \textbf{cutting plane method} and a \textbf{branch-and-bound} \cite{land2010automatic} procedure.
...
The continuous relaxation is solvable in polynomial time, even though the \textbf{simplex algorithm}, which works well in practice, can, in the worst case, take exponential running time.
...
In this thesis, we used the latest \texttt{CPLEX 20.1} C callable library to develop our pricer.
It is clear that using a modern and well developed MIP solver, such as CPLEX, can provide
two benefits compared to custom algorithms which are mainly tailored only at a single problem.
First, using a MIP solver efficient parallel usage of the machine' multiple cores comes for free.
Second, it is possible to re-use all the enginnering effort that went in creating a MIP solver
...
A block diagram summarizing the fractioanl separation is provided in Figure \ref{fig:fractional-separation-block-diagram}.
