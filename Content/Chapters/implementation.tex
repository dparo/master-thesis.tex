...
The complete pseudo-algorithm can be found in \ref{algo:gsec-frac-sep}.
...
\begin{algorithm}
    ...
    \Proc{\textit{gsec\_frac\_sep}($\maxflow(s, t)$, $F_1(s, t)$, $F_2(s, t)$, $x^*$, $y^*$)}{
        \Const $\varepsilon_{\mt{GSEC}} \gets 0.2$\;
        \Let $S$\;
        \uIf{$0 \notin F_1(s, t)$}{
            $S \gets F_1(s, t)$\;
        }\Else{
        ...
        \For{$i \in V \mid i \in S$}{
            \Let $v \gets \maxflow(s, t) - 2 y^*_i$\;

            \If{$\maxflow(s, t) \ge 2 y^*_i - \varepsilon_{\mt{GSEC}}$ \And $v < m$}{
                $c \gets i$\;
                $m \gets v$\;
            }
        }
...
\begin{equation}
	\begin{split}
		\ExprCptpFlowExitingWithWeight{S}{\Expr*{1 - 2 \frac{q_j}{Q}}} - 2 	\ExprCptpServedDemandWithWeight{S}{i}{\frac{q_i}{Q}} \ge - \varepsilon_{\mt{GSEC}}
	\end{split}
\end{equation}
