...
is an optimization software package for solving problems expressed as either:
...
CPEX applies pre-processing at the root to improve the formulation and reduce the problem size.
...
The node-selection and branching decision startegie
...
MIP solvers are quite general and can be employed to solve a variety of problems from different fields.
...
MIP solvers have are exact by nature: given enough running time, they can compute a proven \textbf{optimal} solution by exhibiting both a lower bound and an upper bound to the objective function.
