...
a constructive \textit{insertion heuristic} stage (see \cref{sec:impl-insertion-heuristic})
followed by a \textit{2-OPT refinement} stage (see \cref{sec:impl-2opt-refinement}),
...
The \textit{insertion heuristic} algorithm is an iterative approach
...
In our implementation we chose $u = 0$ and $v \in V_0$ satisfying $q_u + q_v \le Q$.
...
Instead, the exact separation of the RCC and GLM inequalities is a separate concern; for more info see in \cref{sec:cptp-rcc,sec:cptp-glm}.
There is currently no exact separation procedure for the RCC inequalities  \parencite{jepsen2014}.
On the other hand, GLM inequalities can be separated by solving $(u, v=0)$-min-cuts on a flow network with $u$-dependent capacities \parencite{letchford2006, jepsen2014}.
...
\begin{algorithm}
	...
	\input{Content/Chapters/Include/cc-dfs-pseudoalgo.tex}
	\label{algo:cc-dfs}
\end{algorithm}
...
\Cref{algo:gsec-integral-sep} contains the complete pseudocode for the GSEC integral separation procedure.
...
\begin{algorithm}
	...
	\input{Content/Chapters/Include/gsec-integral-sep-pseudoalgo.tex}
\end{algorithm}
...
\Cref{algo:gsec-frac-sep} contains the complete pseudocode for the GSEC fractional separation procedure.
...
\begin{algorithm}
	...
	\input{Content/Chapters/Include/gsec-frac-sep-pseudoalgo.tex}
\end{algorithm}
...
be the network labeling output by the major connected components as discussed in \cref{sec:impl-labeling-integral-solutions}.
...
\Cref{algo:rcc-integral-sep} contains a complete pseudocode.
...
\Cref{algo:rcc-frac-sep} contains the complete pseudocode.
...
\begin{algorithm}
	...
	\input{Content/Chapters/Include/rcc-integral-sep-pseudoalgo.tex}
\end{algorithm}
...
\begin{algorithm}
	...
	\input{Content/Chapters/Include/rcc-frac-sep-pseudoalgo.tex}
\end{algorithm}
...
\Cref{algo:glm-integral-sep} contains a complete pseudocode.

Following that, we describe the separation of the GLM inequalities for fractional solutions.
This separation procedure follows roughly the same reasoning
as the GSEC fractional separation discussed in \cref{sec:impl-gsec-fractional-separation}.
Let $\maxflow(u, v),\ F_1(u, v),\ F_2(u, v)$ denote the maxflow value and bipartitions
produced by the network labeling algorithm described in \cref{sec:impl-labeling-fractional-solutions}.
...
\Cref{algo:glm-frac-sep} contains the complete pseudocode.
...
\begin{algorithm}
	...
	\input{Content/Chapters/Include/glm-integral-sep-pseudoalgo.tex}
\end{algorithm}
...
\begin{algorithm}
	...
	\input{Content/Chapters/Include/glm-frac-sep-pseudoalgo.tex}
\end{algorithm}
