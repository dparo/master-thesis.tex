...
Section \cref{sec:impl-full-static-model} presents the implemented
static IP model, which is based on the CPTP formulation of \cref{sec:the-capacitated-profitable-tour-problem}.
Section \cref{sec:impl-upper-cutoff-value} describes the employed upper cutoff-value,
a technique to reduce the number of branch and bounds nodes used
Section \cref{sec:impl-warm-start} describes the primal heuristics employed
to warm start the CPLEX optimizer.
Section \cref{sec:impl-branching} describes the employed branching schemes.
...
at reducing the number of branch-and-bound nodes
that need to be explored.
The upper cutoff value behaves as a constraint specified on the objective function $z_\mt{CPTP}(x, y)$:
\begin{equation}
